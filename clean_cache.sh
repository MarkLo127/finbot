...
if [ "$VENV_COUNT" -gt 0 ]; then
    echo "  ✓ 刪除 $VENV_COUNT 個 Python 虛擬環境目錄 (venv/.venv/env)"
...
if [ "$MYPY_COUNT" -gt 0 ]; then
    echo "  ✓ 刪除 $MYPY_COUNT 個 .mypy_cache 目錄"
    find . -type d -name ".mypy_cache" -exec rm -rf {} + 2>/dev/null || true
else
    echo "  - 沒有 .mypy_cache 目錄，跳過"
fi

# 清理 .ruff_cache
RUFF_COUNT=$(find . -type d -name ".ruff_cache" 2>/dev/null | wc -l | tr -d ' ')
if [ "$RUFF_COUNT" -gt 0 ]; then
    echo "  ✓ 刪除 $RUFF_COUNT 個 .ruff_cache 目錄"
    find . -type d -name ".ruff_cache" -exec rm -rf {} + 2>/dev/null || true
else
    echo "  - 沒有 .ruff_cache 目錄，跳過"
fi
...
echo "  • Python: __pycache__, *.pyc, *.pyo, .pytest_cache, *.egg-info, venv, .mypy_cache, .ruff_cache"
